...
class QLearningAgent:
    def __init__(self, actions=[0,1,2,3]):
        ...
        self.epsilon = 0.5        	# TODO --> SE MODIFICA PARA LA PREGUNTA 1E y 2B
...
if __name__ == "__main__":
    ...
    for episode in range(NUM_EPISODIOS):
        ...
        prev_q_table = {k: v.copy() for k, v in agent.q_table.items()}  # Copia profunda de la Q-table
        #prev_q_table = agent.q_table.copy()	# Guardar la q-table antes de aprender
        TOLERANCE = 1e-6  # Tolerancia para determinar convergencia
        ...
        if not converged:
            ...
            for key in agent.q_table.keys():
                old_values = prev_q_table.get(key, [DEF_Q_V] * len(agent.actions))
                new_values = agent.q_table[key]
                diff += sum(abs(np.array(new_values) - np.array(old_values)))

            if diff < TOLERANCE and reward > 0:
                converged = True
             ### AQUÍ SE DEBE INTEGRAR LA LÓGICA QUE COMPRUEBE QUE SE HAN CONVERGIDO ##
             
            if converged:
                print(f"El algoritmo ha convergido tras {episode} episodios. Diferencia: {diff} < {TOLERANCE}")
                #converged = True
                break
            #converged = False
